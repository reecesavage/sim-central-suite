...
| `excerpt` | string \| null | Plain-text preview of the body &mdash; HTML flattened, entities decoded, whitespace collapsed, capped at 300 chars (ellipsis included in the cap). `null` when the body is empty. *(v1.36.0+)* Flattening is **block-boundary aware** *(v1.36.1+)*: `</p>`, `<br>`, `</h2>`, `</li>` and table cells become one space, inline tags become nothing, escaped markup is removed as markup, a `<` in prose (`in <20 minutes`) is preserved, `script`/`style` contents are dropped, and truncation counts characters so the value is always valid UTF-8. Same code and same cap as the snapshot's excerpts. |
...
| `word_count` | int | Words in this post's body, counted after the same flattening the `excerpt` above describes — the suite's one canonical definition, and the same figure Manage Missions and `/sim/missions` show. *(v1.37.0 rewrote this: the body is flattened rather than `strip_tags`'d, and tokens are counted with a Unicode-aware matcher rather than `str_word_count`. So a block boundary separates words instead of fusing them, a bare `<` in prose no longer swallows the rest of the body, `&nbsp;` no longer counts as the word "nbsp", a pasted `<style>` block's CSS no longer counts as prose, an accented word like `Björn` counts once instead of twice, a hyphenated or apostrophised word counts once, and digits count. Counts moved on every sim as a result — read the field, don't reimplement it.)* Computed per request rather than stored, so it is present for **every** status (drafts included), never `null`, and still present when `?content=0` omits the body. Not attributed to any single author — a post can have several. *(v1.28.0+)* |
