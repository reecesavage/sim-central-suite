...
<p align="center">
  <a href="https://github.com/reecesavage/sim-central-suite/releases/tag/v1.40.0"><img src="https://img.shields.io/badge/Version-v1.40.0-brightgreen.svg"></a>
...
- **REST API** *(v1.9.1+)* &mdash; HTTP API for external integrations (n8n flows, scripts, dashboards, mobile apps); admin-issued bearer tokens with per-scope grants (`posts:read`, `characters:read`, `missions:read`, `positions:read` *(v1.22.0+)*, `logs:read` *(v1.39.0+)*, plus write scopes `users:write`, `webhooks:read`, `webhooks:write` *(v1.14.0+)* and post-authoring scopes `posts:read.own` / `posts:write` / `posts:delete` + `*.all` sysadmin variants *(v1.15.0+)*, and `tokens:read` / `tokens:write` *(v1.16.0+)*, and `suite:update` *(v1.21.0+)*); read endpoints for posts/characters/missions and **top open positions** *(v1.22.0+)*, **character bio fields** on the single-character read *(v1.38.0+)*, further **API improvements** *(v1.39.0+)* &mdash; a personal logs endpoint, resolved rank and position on characters, and change tracking so a consumer can fetch only what changed &mdash; `GET /me` identity, **post authoring** (create/update/delete, save or activate) for user-bound tokens, collaborative **post edit locking** *(v1.19.0+)*, **API token management** (list/create/revoke/delete, sysadmin-bound) *(v1.16.0+)*, **remote suite upgrades** via `GET`/`POST /suite` *(v1.21.0+)*, plus write endpoints to disable/reactivate users + their characters and manage event webhooks; per-token rate limiting; one-time-display token reveal; surface suite-feature fields (summary, ordered post day/time, display name, etc.) in the JSON when those features are on
- **Mobile Site** *(v1.17.0+)* &mdash; a lightweight, phone-friendly site at `/mobile` where members log in and write, save, post, edit, or delete their own mission posts and personal logs, and (v1.29.0+) read and send **private messages**, without fighting Nova's desktop views on a phone. Honours Discord Sign-In settings on login (Discord-only sims show the Discord button, not a password form), enforces Nova's permissions, and reuses the suite's post engine so webhooks/emails/ordered-timelines/moderation behave identically to the desktop site.
- **Event Webhooks** *(v1.11.0+)* &mdash; fire HTTP webhooks when posts are saved, activated, or edited after activation (`post.updated` *(v1.32.0+)*). Discord-formatted (rich embed with @mentions of linked authors via stored Discord IDs) or generic JSON (for n8n etc.). Multiple webhooks per event, per-webhook event subscriptions, customisable Discord embed templates for `post.posted`, `log.posted`, and `news.posted` *(v1.13.0+)*, an optional per-event Discord role ping *(v1.13.0+)*, **Test** button for each webhook, fire-and-forget delivery with status visibility (last_status + last_error per webhook).
...
Adds two columns to `users`: `nova_ext_discord_auth_id` (UNIQUE-indexed Discord snowflake) and `_linked_at`. That's the whole storage footprint &mdash; the public Discord ID and when it was linked. Username, avatar, and email are never persisted. Configure on the *Discord Sign-In &rarr; Configure* page:
...
- **Request Discord email** *(v1.25.0+)* &mdash; off by default. The suite sends v2 requests to the broker (v1.2.0+), which skip Discord's `email` scope entirely; the consent screen only asks for basic identity (plus servers when a guild check is on). Turn this on to request the email scope so the join form's email field pre-fills from Discord &mdash; the address is shown to the user, never stored. When on, sign-in also requires the Discord email to be verified (enforced at both the broker and the suite as defense-in-depth).
...
- **Sign in with Discord** button on the login form, with a **Remember me** checkbox *(v1.25.0+)* &mdash; logs the user in if their Discord ID is already linked to a sim account; ticking the box sets Nova's standard 14-day remember-me cookies so the session survives browser restarts, exactly like the password form's checkbox.
...
- **Personal logs**: the same list / write / save / post / edit / delete flow for the member's own logs.
- **Messages (DMs)** *(v1.29.0+)*: inbox with an unread badge in the nav, read a message (marked read on open), reply, compose a new message to any active member, and delete (soft, per-side) from the inbox or Sent. Reuses Nova's own private-message system, so messages sent from mobile appear in the desktop inbox and vice versa. Honours Nova's `messages/index` access control — roles without messaging see no Messages tab. In-app delivery only (no notification email from the mobile compose yet).
...
- **Off by default**; enable under *Sim Central Suite &rarr; Mobile Site*. Covers mission posts, personal logs, and messages; news may follow.
...
- **Edit scopes** *(v1.28.0+)* &mdash; change an existing token's scopes in place without re-issuing the key. Click the scope list on the token row to expand a checkbox editor, tick/untick, and **Save scopes**. Re-validated against the token's existing user binding, so you still can't grant a post `read.own`/`write`/`delete` scope to an unbound token. (The managed Sim Central access token is excluded &mdash; change its access via Revoke / re-grant.)
...
| `GET`/`POST`/`PUT`/`DELETE` | `/posts/{id}/lock` *(check / acquire / heartbeat / release; `PATCH /posts/{id}` returns `423` if locked)* | `posts:write` |
...
| `GET` | `/snapshot` *(Astrolabe snapshot: game, manifest, stories, recent posts, counts)* | `astrolabe:read` |
...
| `GET` | `/suite` *(installed + latest version, update available)* | any valid token |
| `POST` | `/suite` *(run the updater; legacy `/suite/update` alias)* | `suite:update` *(sysadmin-bound)* |
...
**Token management** *(v1.16.0+)*: list/create/revoke/delete API tokens over the API (`tokens:read` / `tokens:write`), the same actions as the ACP token page. `PATCH /tokens/{id}` also **edits scopes** *(v1.28.0+)* &mdash; send a `scopes` array to replace the set in place (re-validated against the token's user binding); send `revoked` to toggle revocation; either or both. `POST /tokens` can bind by **`discord_id`** *(v1.32.0+)* as an alternative to `user_id` (the linked Discord account; requires the Discord Auth feature, mirroring `/users/disable`). Every token endpoint requires the calling token to be bound to a **sysadmin** user (`403` otherwise), mirroring the ACP's `site/settings` gate. Create returns the raw token exactly once; the hash is never exposed. Treat a `tokens:write` token as highly privileged &mdash; it can mint tokens with any scope.
...
**Post edit locking** *(v1.19.0+)*: `GET` / `POST` / `PUT` / `DELETE /posts/{id}/lock` give a token (with `posts:write` + a bound user) a 10-minute collaborative lock &mdash; check, acquire, heartbeat, release &mdash; so two writers don't clobber the same post. Acquire is idempotent for the owner and returns `409` (with the current owner's name + expiry) when someone else holds a fresh lock; a `posts:write.all` sysadmin token can force-release. `PATCH /posts/{id}` honours locks: `423` if another user holds one, and your own lock auto-releases on a successful save.

**Suite self-management** *(v1.21.0+)*: `GET /suite` reports the installed version, the latest published release, and whether an update is available (any valid token). `POST /suite` runs the one-click updater &mdash; default to the newest release or pass a `version` &mdash; gated on the `suite:update` scope **and** a sysadmin-bound token, so Sim Central (or your own automation) can roll out upgrades without an admin logging into the ACP. Routing is by HTTP method; the legacy `POST /suite/update` still works.

Every post carries a **`word_count`** *(v1.28.0+)* &mdash; the words in that post's body, counted after the same flattening the API's `excerpt` uses and matching the counts shown on Manage Missions and `/sim/missions` &mdash; *rewritten in v1.37.0 (flattened rather than `strip_tags`'d, and tokenised with a Unicode-aware matcher rather than `str_word_count`), which moved the numbers on every sim*. Missions carry a **`word_count`** too (total across the mission's activated posts), but only for tokens that can read all posts (`posts:read` / `posts:read.all`), since a mission total aggregates every author's posts; own-only or missions-only tokens don't get it. Word counts are per-post/per-mission &mdash; they are **not** attributed to individual authors (a post can have several).
...
**Astrolabe snapshot** *(v1.30.0+)*: `GET /snapshot` (scope `astrolabe:read`) returns one read-only aggregate of the sim's **public** data &mdash; game info, crew manifest (departments + characters, active players and NPCs), stories (missions), the 10 most recent posts, **open positions** *(v1.31.0+)*, and headcounts &mdash; for the Astrolabe platform to mirror on its per-game page. All URLs/avatars/rank images are absolute https or null; descriptions/excerpts are flattened to plain text (block boundaries preserved as spaces, so paragraphs don't run together &mdash; *v1.36.1+*) and length-capped; no private data. Served from a short (~10 min) cache since Astrolabe polls on a schedule. A token scoped to only `astrolabe:read` exposes just this one endpoint. Full integration contract (for the Astrolabe developer) in [`ASTROLABE.md`](ASTROLABE.md).
...
- **Unlink everyone's Discord at once**: `UPDATE nova_users SET nova_ext_discord_auth_id = NULL, nova_ext_discord_auth_linked_at = NULL;` &mdash; useful if you ever rotate the broker (changing public keys would invalidate every existing token but already-linked rows would still work for matching).
